...
from zero_vibration_pvt_sequence_generator import ShaperType
...
class ShaperConfig:
    ...
    def shaper_mode(self, mode: ShaperMode) -> None:
        ...
        match self.shaper_mode:
            ...
            case ShaperMode.PVT:
                self.settings = Settings(shaper_type=ShaperType.ZV, pvt_id=1)
...
if __name__ == "__main__":
    config_decel = ShaperConfig(ShaperMode.DECEL)
    config_pvt = ShaperConfig(ShaperMode.PVT)
    config_pvt = ShaperConfig(ShaperMode.PVT, shaper_type=ShaperType.ZVD)
    config_decel = ShaperConfig(ShaperMode.DECEL, shaper_type=ShaperType.ZVD)
